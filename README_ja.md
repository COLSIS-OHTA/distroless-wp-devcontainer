...
- コンテナ名には `COMPOSE_PROJECT_NAME`（既定: `wp`）が接頭辞として付与されます。変更はリポジトリルートの `.env` で行い、`docker compose -p wp down` のあと `docker compose up -d` または DevContainer の **Rebuild Container** が必要です（Reopen だけでは変わりません）。
